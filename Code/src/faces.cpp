...
// Short weather string for the faces: "11.7C", or "--" when nothing has
// arrived. Each face renders it in its own idiom.
static const char* wxShort() {
  static char buf[10];
  const WeatherData& w = Link::weather();
  if (!w.valid) { strcpy(buf, "--"); return buf; }
  snprintf(buf, sizeof(buf), "%.1fC", w.tempC10 / 10.0f);
  return buf;
}
static const char* wxWord() {
  const WeatherData& w = Link::weather();
  return w.valid ? Link::iconName(w.icon) : "no data";
}
// Event name if one falls today, else nullptr. Stays up all day until the
// event is removed from the calendar.
static const char* eventToday() { return Cal::todayName(); }
...
static void faceBold(const DateTime& n) {
  ...
  const char* ev = eventToday();
  if (ev) {
    // bold face gets a bold banner: inverted bar across the date line
    int16_t tw = (int16_t)display.getStrWidth(ev) + 8;
    display.drawBox(64 - tw / 2, 46, tw, 12);
...
static void faceSeconds(const DateTime& n) {
  ...
  display.setFont(F_BODY);
  const char* ev2 = eventToday();
  uiLeft(ev2 ? ev2 : d, 2, 54);

  display.setFont(F_TINY);
  uiRight(wxShort(), 126, 54);
...
static void faceAnalog(const DateTime& n) {
  ...
  display.setFont(F_TINY);
  uiCentre(l3, 102, 52);
  uiCentre(wxShort(), 102, 61);

  const char* ev = eventToday();
  if (ev) {
    // engraved into the dial, like a date window
    display.setDrawColor(0);
    display.drawBox(14, 48, 52, 11);
    display.setDrawColor(1);
    display.drawFrame(14, 48, 52, 11);
    display.setFont(F_TINY);
    uiCentre(ev, 40, 53);
  }
...
static void faceSquare(const DateTime& n) {
  ...
  char d[24];
  const char* ev = eventToday();
  if (ev) snprintf(d, sizeof(d), "%s", ev);
  else    snprintf(d, sizeof(d), "%s %02d", DOW[n.dayOfTheWeek()], n.day());
  display.setFont(F_TINY);
  display.setDrawColor(0);
  ...
  // weather in the top-left corner of the panel, also punched out
  display.setDrawColor(0);
  display.drawBox(4, 4, display.getStrWidth(wxShort()) + 4, 9);
  display.setDrawColor(1);
  display.drawStr(6, 11, wxShort());
...
static void faceSpace(const DateTime& n) {
  ...
  // plain temperature, punched out of whatever is behind it. wxShort() already
  // gives "--" when nothing has arrived from the phone.
  display.setFont(F_TINY);
  int16_t ww = (int16_t)display.getStrWidth(wxShort());
  display.setDrawColor(0);
  display.drawBox(124 - ww - 2, 48, ww + 4, 9);
  display.setDrawColor(1);
  uiRight(wxShort(), 124, 53);

  const char* ev = eventToday();
  if (ev) {
    char msg[26];
    snprintf(msg, sizeof(msg), "<< %s >>", ev);
    display.setDrawColor(0);
...
static void faceSevenSeg(const DateTime& n) {
  ...
  char d[28];
  const char* ev = eventToday();
  if (ev) snprintf(d, sizeof(d), "* %s *  %s", ev, wxShort());
  else    snprintf(d, sizeof(d), "%s %02d %s  %s", DOW[n.dayOfTheWeek()], n.day(),
                   MON[n.month() - 1], wxShort());
  display.setFont(F_TINY);
  uiCentre(d, 64, 59);
...
static void faceTerminal(const DateTime& n) {
  ...
  const char* ev = eventToday();
  if (ev) snprintf(l, sizeof(l), "> event: %.14s", ev);
  else    snprintf(l, sizeof(l), "> wx %s %.10s", wxShort(), wxWord());
...
static void faceMatrix(const DateTime& n) {
  ...
  char l[26];
  const char* ev = eventToday();
  if (ev) snprintf(l, sizeof(l), "?? EVENT: %.12s", ev);
  else    snprintf(l, sizeof(l), "root@watch %s %s", wxShort(),
                   (millis() / 400) & 1 ? "_" : "");
  display.setFont(F_TINY);
  display.setDrawColor(0);
