...
void Btn::update() {
  ...
  // Single presses. Both buttons fire as soon as the combo window has passed,
  // which is the only delay in the path - just long enough to tell a single
  // press from a deliberate both-button press.
  for (uint8_t i = 0; i < 2; i++) {
