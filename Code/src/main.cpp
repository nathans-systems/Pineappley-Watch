...
void setup() {
  ...
  goState(ST_FACE);
  Btn::markActivity();
  Serial.printf("[BOOT] fw=%s cpu=%luMHz face=%s standby=%lus deepsleep=%lus ble=%s\n",
                FW_VERSION, (unsigned long)getCpuFrequencyMhz(), FACES[gFace].name,
                (unsigned long)(IDLE_SCREEN_OFF_MS / 1000UL),
                (unsigned long)(DEEP_SLEEP_AFTER_MS / 1000UL),
                ENABLE_BLE ? "on" : "off");
}
...
void loop() {
...
#if DEEP_SLEEP_AFTER_MS
    if ((millis() - Btn::lastActivity()) > DEEP_SLEEP_AFTER_MS) goToSleep();
#endif
  ...
  if ((millis() - Btn::lastActivity()) > IDLE_SCREEN_OFF_MS) {
    saveSettings();
    screenStandby();
  }
