...
// A running stopwatch always blocks deep sleep, otherwise the count would be
// lost. This decides whether it also keeps the screen lit. 0 lets the display
// dim while the stopwatch keeps counting in the background.
#define STOPWATCH_KEEPS_SCREEN_ON  1
...
// Double-tap B1 to select. The first tap is emitted immediately, so there is no
// navigation lag; a second tap inside this window sends EV_SELECT2 and the
// screen rolls its cursor back. 0 disables it.
